...
def read_frame(
    ...
    pairs = find_pairs(find_candidates(gray))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Lê cartas a partir de vídeo, frame ou diretório.")
    parser.add_argument("source", type=Path, help="vídeo, PNG ou diretório de PNGs")
    ...
    try:
        for name, image in iter_frames(args.source):
            readings = read_frame(image, ranks, suits, args.threshold)

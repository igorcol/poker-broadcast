...
const RANK_VALUES: ReadonlyMap<Rank, number> = new Map(
...
export function isCard(value: string): value is Card {
  return (
    value.length === 2 &&
    RANK_SET.has(value.charAt(0)) &&
    SUIT_SET.has(value.charAt(1))
  );
}
...
export function parseCard(value: string): Card | null {
  ...
  const candidate =
    normalized.charAt(0).toUpperCase() + normalized.charAt(1).toLowerCase();
  return isCard(candidate) ? candidate : null;
}

/** Lê uma sequência de cartas com ou sem separador: `"KsQh"`, `"Ks Qh"`, `"10h 2c"`. */
export function parseCards(input: string): Card[] | null {
  const compact = input.trim().toLowerCase().replace(/\s+/g, "");
  const tokens = compact.match(/(?:10|[2-9tjqka])[shdc]/g);

  // Exige consumo total: "xyKsQh" não pode passar só porque contém duas cartas válidas
  if (tokens === null || tokens.join("") !== compact) return null;

  const cards = tokens
    .map(parseCard)
    .filter((card): card is Card => card !== null);
  return cards.length === tokens.length ? cards : null;
}

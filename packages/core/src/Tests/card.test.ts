...
describe("parseCards", () => {
  it("lê sequência com e sem separador", () => {
    assert.deepEqual(parseCards("KsQh"), ["Ks", "Qh"]);
    assert.deepEqual(parseCards("ks qh"), ["Ks", "Qh"]);
    assert.deepEqual(parseCards("10h 2c 3d"), ["Th", "2c", "3d"]);
  });

  it("recusa entrada com lixo no meio", () => {
    for (const value of ["", "xy", "KsX", "Ks Q", "KsQhZ"]) {
      assert.equal(parseCards(value), null, `${value} deveria falhar`);
    }
  });
});

...
describe("startHand", () => {
  ...
  it("inverte os blinds em heads-up: botão paga o small e age primeiro", () => {
    const state = startHand({
      ...THREE_HANDED,
      seats: THREE_HANDED.seats.slice(0, 2),
    });
...
describe("rodada de apostas", () => {
  ...
  it("avança para o flop quando o big blind fecha a rodada", () => {
    ...
    assert.equal(
      state.toAct,
      1,
      "pós-flop começa no primeiro ativo depois do botão",
    );
  });
  ...
  it("reabre a ação para quem já tinha agido quando alguém sobe", () => {
    ...
    assert.equal(state.toAct, 2);
    assert.equal(
      state.seats[0]?.hasActed,
      false,
      "quem pagou precisa responder ao raise",
    );
...
describe("validação", () => {
  ...
  it("recusa raise abaixo do mínimo", () => {
    const result = applyAction(startHand(THREE_HANDED), {
      type: "raise",
      to: 30,
    });
  ...
  it("recusa raise com valor não numérico ou negativo", () => {
    const state = startHand(THREE_HANDED);

    for (const to of ["abc", null, undefined, NaN, Infinity, -500, 0, 40.5]) {
      const result = applyAction(state, {
        type: "raise",
        to,
      } as unknown as Action);
      assert.equal(result.ok, false, `to=${String(to)} deveria ser recusado`);
      assert.equal(
        result.ok === false && result.error,
        "invalid-amount",
        `to=${String(to)} com erro errado`,
      );
    }
  });
});
